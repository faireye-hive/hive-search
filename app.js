...
async function fetchAllUserPosts(username) {
  try {
    ...
    if (
      !firstPageData.permlinks_result ||
      firstPageData.permlinks_result.length === 0
    ) {
      return {
        stubs: [],
        totalPosts: 0,
    ...
    totalPosts =
      firstPageData.total_permlinks || firstPageData.permlinks_result.length;
    ...
    if (totalPages > 1) {
      ...
      for (let page = 2; page <= totalPages; page++) {
        try {
          ...
          if (
            pageData.permlinks_result &&
            pageData.permlinks_result.length > 0
          ) {
...
function removeFilter(type, value) {
  switch (type) {
    case "author":
      currentSearch.filters.authors = currentSearch.filters.authors.filter(
        (a) => a !== value
      );
      break;
    case "include":
...
function applyFilters() {
  ...
  if (currentSearch.filters.includeTerms.length > 0) {
    filtered = filtered.filter((post) => {
      ...
      const hasCategoryMatch = currentSearch.filters.includeTerms.some((term) =>
      ...
      if (post.json_metadata) {
        try {
          ...
          if (metadata.tags && Array.isArray(metadata.tags)) {
            ...
            hasTagMatch = currentSearch.filters.includeTerms.some((term) =>
  ...
  if (currentSearch.filters.excludeTerms.length > 0) {
    filtered = filtered.filter((post) => {
      // Verifica categoria
      const category = post.category ? post.category?.toLowerCase() : "";
      const hasCategoryExclusion = currentSearch.filters.excludeTerms.some(
      ...
      if (post.json_metadata) {
        try {
          ...
          if (metadata.tags && Array.isArray(metadata.tags)) {
            const tags = metadata.tags.map((tag) => tag?.toLowerCase());
            const hasTagExclusion = currentSearch.filters.excludeTerms.some(
...
function applyLocalFilter() {
  ...
  const adv = currentSearch.advancedFilters;
  ...
  if (!hasBasicFilter) {
    ...
    return updateLocalFilterStats();
  }

  const searchTerms = filterText
    ? filterText.split(/\s+/).filter(Boolean)
    : [];

  const startDate = adv.dateStart ? new Date(adv.dateStart) : null;
  const endDate = adv.dateEnd
    ? new Date(adv.dateEnd + "T23:59:59")
    : null;
  const categoryFilter = adv.category
    ? adv.category.toLowerCase()
    : null;

  const filtered = currentSearch.allPosts.filter(post => {

    // -----------------------------------------------------
    // 1) FILTRO DE TEXTO (rápido e seguro)
    // -----------------------------------------------------
    if (searchTerms.length > 0) {
      ...
      for (const term of searchTerms) {
        ...
        // Ano (4 dígitos)
        if (/^\d{4}$/.test(term)) {
          const year = post.created
            ? new Date(post.created).getFullYear().toString()
            : "";
          if (!year.includes(term)) return false;
          continue;
        }
        ...
        if (term.startsWith("#")) {
          ...
          if (!tags.some(tag => tag.includes(t))) return false;
          continue;
        }
...
function firstImageFromBody(body) {
  ...
  const url = /(https?:\/\/[^\s"'()]+\.(?:png|jpg|jpeg|gif|webp|bmp))/i.exec(
    body
  );
  if (url) return url[1];
...
function sortPosts(posts) {
  if (currentSearch.sortOrder === "recent") {
    ...
    if (currentSearch.searchMode === "similar") {
      ...
      return [...posts].sort((a, b) => {
        const indexA = orderMap.get(`${a.author}/${a.permlink}`) ?? Infinity;
        const indexB = orderMap.get(`${b.author}/${b.permlink}`) ?? Infinity;
        return indexA - indexB;
      });
    }
  }
          ...
          <div class="flex flex-col md:flex-row gap-4">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="mt-1 text-sm text-slate-600">
                ...
                Votos: ${post.active_votes.length || 0}
              </div>
...
function renderCurrentPage() {
  if (
    ...
    if (currentSearch.isFetchingComplete && currentSearch.allPosts.length > 0) {
